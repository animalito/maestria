...
setwd("~/Documents/maestria animalito/parcial_multivariada")
library(dplyr)
library (ggplot2)
library(Hmisc)
...
```{r}
datos <- mkt_train
datos$age <- cut2(datos$age,g=3)

datos$balance <- cut2(datos$balance, cuts=c(0,1500) )

datos$month <- car::recode(datos$month,
              recodes=paste0("'",levels(datos$month),"'=",c(4,8,12,2,1,7,6,3,5,11,10,9),collapse=';') )  

datos$month <- cut2(as.numeric(as.character(datos$month)), cuts=c(4,7,10))

levels(datos$month) <- c('Trim-1','Trim-2','Trim-3','Trim-4')

datos$duration <- cut2(datos$duration,g=3)
levels(datos$duration) <- c('baja duracion','media duracion' , 'alta duracion')

datos$campaign <- cut2(datos$campaign,g=3)
levels(datos$campaign) <- c('uno','dos a tres','cuatro o mas')

datos$previous <- cut2(datos$previous,g=2)
levels(datos$previous) <- c('no','yes')
...
muestras bootstrap (a partir de los datos de entrenamienti), y calcula una red
...
*¿Coinciden los valores de los incisos anteriores para todas las variables?, 
¿Cuál es la diferencia entre los dos métodos?, ¿Cuándo usarías uno u otro? 
